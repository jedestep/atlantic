...
class ConfirmMenu(state.State):
    ...
    def draw(self, screen):
        ...
        surf = self._font.render("This will overwrite your", True, self._color)
        h = surf.get_size()[1]
        y = 0 * height / 4 - h / 2 + self._size
        screen.blit(surf, (30, y))

        surf = self._font.render("current save", True, self._color)
        h = surf.get_size()[1]
        y = .5 * height / 4 - h / 2 + self._size
        screen.blit(surf, (200, y))

        surf = self._font.render("Continue?", True, self._color)
        h = surf.get_size()[1]
        y = 1.3 * height / 4 - h / 2 + self._size
        screen.blit(surf, (x, y))

        surf = self._font.render("Yes", True, self._color)
        h = surf.get_size()[1]
        y = 2.1 * height / 4 - h / 2 + self._size
        self._heights.append(y)
        screen.blit(surf, (x, y))

        surf = self._font.render("No", True, self._color)
        h = surf.get_size()[1]
        y = 2.9 * height / 4 - h / 2 + self._size
        self._heights.append(y)
        screen.blit(surf, (x, y))
